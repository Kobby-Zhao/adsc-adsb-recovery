...
RUN_TAG="${RUN_TAG:-bimamba_coarsetrend_5ep_v1}"
...
cfgs = {
    ...
    "D_bimamba_context_xyaux_zlinear_zadapter_gapaware_small_coarsetrend": build_cfg(
        "D_bimamba_context_xyaux_zlinear_zadapter_gapaware_small_coarsetrend",
        "bimamba_context_xyaux_zlinear_zadapter_gapaware_small_coarsetrend",
        "BiMamba xyaux + zlinear + gap-aware small z adapter + coarse trend residual, 5-epoch sanity.",
    ),
}
...
MODELS=(B_bimamba_context_xyaux_zlinear_zadapter_gapaware_small D_bimamba_context_xyaux_zlinear_zadapter_gapaware_small_coarsetrend)
if [[ -n "${ONLY:-}" ]]; then
...

{
  ...
  echo "ONLY=${ONLY:-<default B_bimamba_context_xyaux_zlinear_zadapter_gapaware_small,D_bimamba_context_xyaux_zlinear_zadapter_gapaware_small_coarsetrend>}"
  echo "START=$(date -Is)"
} > "${SUMMARY_FILE}"
