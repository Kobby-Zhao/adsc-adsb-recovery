...
RUN_TAG="${RUN_TAG:-bimamba_zadapter_gapgate_5ep_v1}"
...
def build_cfg(name: str, backbone_type: str, note: str) -> dict:
...
cfgs = {
    "B_bimamba_context_xyaux_zlinear_zadapter": build_cfg(
        "B_bimamba_context_xyaux_zlinear_zadapter",
        "bimamba_context_xyaux_zlinear_zadapter",
        "BiMamba xyaux + zlinear + standard zadapter (d//4), 5-epoch sanity.",
    ),
    "D_bimamba_context_xyaux_zlinear_zadapter_gapgate": build_cfg(
        "D_bimamba_context_xyaux_zlinear_zadapter_gapgate",
        "bimamba_context_xyaux_zlinear_zadapter_gapgate",
        "BiMamba xyaux + zlinear + gap-gated zadapter (d//4), 5-epoch sanity.",
    ),
}
...
MODELS=(B_bimamba_context_xyaux_zlinear_zadapter D_bimamba_context_xyaux_zlinear_zadapter_gapgate)
if [[ -n "${ONLY:-}" ]]; then
...

{
  ...
  echo "ONLY=${ONLY:-<default B_bimamba_context_xyaux_zlinear_zadapter,D_bimamba_context_xyaux_zlinear_zadapter_gapgate>}"
  echo "START=$(date -Is)"
} > "${SUMMARY_FILE}"
