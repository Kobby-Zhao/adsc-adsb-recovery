...
RUN_TAG="${RUN_TAG:-bimamba_vprogaux_5ep_v1}"
...
def build_cfg(name: str, backbone_type: str, note: str, lambda_vprog: float) -> dict:
    ...
    deep_update(
        cfg,
        {
            ...
            "loss": {
                "lambda_vprog": lambda_vprog,
                "vprog_enable_abs_dz_min": 100.0,
            },
            "training": {
                "epochs": epochs,
            },
...
cfgs = {
    ...
    "E_bimamba_context_xyaux_zlinear_zadapter_gapaware_small_vprogaux": build_cfg(
        "E_bimamba_context_xyaux_zlinear_zadapter_gapaware_small_vprogaux",
        "bimamba_context_xyaux_zlinear_zadapter_gapaware_small_vprogaux",
        "BiMamba xyaux + zlinear + gap-aware small z adapter + vertical progress auxiliary supervision, 5-epoch sanity.",
        0.02,
    ),
}
...
MODELS=(B_bimamba_context_xyaux_zlinear_zadapter_gapaware_small E_bimamba_context_xyaux_zlinear_zadapter_gapaware_small_vprogaux)
if [[ -n "${ONLY:-}" ]]; then
...

{
  ...
  echo "ONLY=${ONLY:-<default B_bimamba_context_xyaux_zlinear_zadapter_gapaware_small,E_bimamba_context_xyaux_zlinear_zadapter_gapaware_small_vprogaux>}"
  echo "START=$(date -Is)"
} > "${SUMMARY_FILE}"
